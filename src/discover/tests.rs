...
fn load_miss_suggests_one_close_hyphen_name() {
    ...
    let insert = unknown_or_skipped_skill_named("reviewpr", &report.skips, names.iter().copied());
    assert!(
    ...
    let swap = unknown_or_skipped_skill_named("review_pr", &report.skips, names.iter().copied());
    assert!(
    ...
    let hostile =
        unknown_or_skipped_skill_named("reviewpr\u{2028}x", &report.skips, names.iter().copied());
...
fn take_read_skill_md_paths() -> Vec<PathBuf> {
    super::take_read_skill_md_paths()
}
...
fn extra_path_symlink_to_fifo_is_unreadable_not_hang() {
    ...
    assert!(
        report.skips.iter().any(|s| {
            s.kind == SkipKind::Unreadable && s.path.ends_with("demo/SKILL.md") && s.name.is_none()
        }),
...
#[test]
fn user_dir_whitespace_dotdot_does_not_scan_filesystem_root() {
    // Sibling of extra_path_whitespace_dotdot_does_not_scan_filesystem_root
    // on the user_dir door. expand_user_skills_dir already calls
    // host_token_collapses, so `/ ..` must not rewrite to `/`.
    // Record the same unreadable collapse skip extra-path records.
    let cwd = tempfile::tempdir().expect("cwd");
    let home = tempfile::tempdir().expect("home");
    for raw in [
        " /..",
        ...
        assert!(
            report.skills.is_empty(),
            "user_dir {raw:?} must not load root packages: {:?}",
            report.skills
        );
        assert!(
            report
            ...
            "user_dir {raw:?} must skip as collapse, not walk /: {:?}",
            report.skips
        );
        let dirs = with_home_override(Some(home.path().to_path_buf()), || {
            watch_dirs(cwd.path(), &opts)
        ...
        assert!(
            dirs.is_empty(),
            "user_dir {raw:?} must not watch a collapsed token (same as extra-path): {dirs:?}"
        );
    }
}
